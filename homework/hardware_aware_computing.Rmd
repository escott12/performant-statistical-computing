...
r_results <- ar_precision_matvec_R(v, auto_corr)
rcpp_results <- ar_precision_matvec(v, auto_corr)

# Index differences
index_false <- ar_precision_matvec_R(v, auto_corr) == ar_precision_matvec(v, auto_corr)

# Look at values that aren't the same from the two methods
View(matrix(c(r_results[!index_false], rcpp_results[!index_false]), ncol = 2))

# Last two values don't match; other mismatches due to precision?
tail(r_results)
tail(rcpp_results)
```
...
```{r}
# Compare performances of the R and Rcpp implementation
